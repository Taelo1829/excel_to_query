...
const UploadIcon = () => (
  <svg
    style={{ width: "3rem", height: "3rem", margin: "auto", color: "#9ca3af" }}
    stroke="currentColor"
    fill="none"
    viewBox="0 0 48 48"
    aria-hidden="true"
...
const UpdateIcon = () => (
  <Icon
    path={
      <>
        <path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.73-2.73L3 14"></path>
        <path d="M3 21v-6h6"></path>
        <path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.73 2.73L21 10"></path>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    script.onerror = () =>
      setError(
        "Failed to load the Excel processing library. Please refresh the page."
      );
        ...
        .map((row) => {
          ...
          placeholders.forEach((placeholder) => {
            ...
            if (cellValue !== undefined && cellValue !== null) {
              ...
              queryForRow = queryForRow.replace(
  ...
  const processFile = useCallback(
    (file) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (rows.length === 0) {
            setError(
              "The selected Excel sheet is empty or has no data after the first row."
            );
            setExcelData(null);
            setIsProcessing(false);
            return;
          ...
          setError(
            "An error occurred while processing the file. Please ensure it is a valid Excel file."
          );
          setExcelData(null);
          setIsProcessing(false);
        }
    ...
    [customQuery, generateQueries, resetState]
  );
  ...
  const updateButtonStyle = getButtonStyle(
    styles.updateButton,
    styles.updateButtonHover,
    isUpdateHovered,
    updateBtnDisabled
  );

  const copyBtnDisabled = !sqlQueries;
  const copyButtonStyle = getButtonStyle(
    styles.copyButton,
  ...
  const clearButtonStyle = getButtonStyle(
    styles.clearButton,
    ...
    <div style={styles.container}>
      <div style={styles.innerContainer}>
        <header style={styles.header}>
          ...
          <p style={styles.p}>
            Define a query template, upload an Excel file, and generate your SQL
            script.
          </p>
        </header>
        ...
        <main style={styles.main}>
          ...
          <div style={{ ...styles.section, marginBottom: 0 }}>
            ...
            {!isScriptLoaded && (
              <p style={{ ...styles.errorText, color: "#fde047" }}>
          ...
          {(sqlQueries || isProcessing) && (
            <div
              style={{ ...styles.section, marginTop: "2rem", marginBottom: 0 }}
            >
